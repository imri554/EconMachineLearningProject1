...
data = pd.read_excel('EconMachineLearningProject1/data/financialratiosTech.xlsx')

data = data.drop(columns=['Historical CRSP PERMNO Link to COMPUSTAT Record', 'Fiscal year end', 'Date'])
#data["date"] = pd.to_datetime(data["Fiscal quarter end"])
data = data.drop(columns=["Fiscal quarter end"])

yValues = data["Personal Income (Table 2.1.)"].values

oh_data = pd.get_dummies(data, columns=["Ticker Symbol"])

data = oh_data.to_numpy()
...
#print(data.columns)
#test train split
#x_train, x_test, y_train, y_test = train_test_split(data, yValues, test_size=0.0, random_state=42)

x_train, x_test, y_train, y_test = data, data, yValues, yValues
...
model.add(tf.keras.layers.LSTM(units=128, input_shape=(time_steps, features), return_sequences = True, kernel_regularizer=tf.keras.regularizers.l2(0.01),  recurrent_regularizer=tf.keras.regularizers.l2(0.01),
